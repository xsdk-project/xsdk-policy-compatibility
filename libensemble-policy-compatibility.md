...
|**M3.** Employ user-provided MPI communicator (no MPI_COMM_WORLD). |Full as of v0.5.2|libEnsemble takes an MPI communicator as an option; if libEnsemble is configured for MPI mode, this provided communicator will be employed. If no communicator is given, MPI_COMM_WORLD is taken as a default. |
...
|**R5.** Provide a mechanism to export ordered list of library dependencies. |Full as of v0.5.2| The required dependencies for libEnsemble are given in `setup.py` and a function could be created in `setup.py` that returns these. If installing through pip, these will automatically be installed if they do not exist. There are various optional dependencies, however.|
